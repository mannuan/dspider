...
page_shop_1 = Page(name='大众点评购物店铺列表页面', fieldlist=fl_shop1, listcssselector=ListCssSelector(list_css_selector='#shop-all-list > ul > li'), mongodb=Mongodb(db=TravelDriver.db, collection=TravelDriver.shop_collection), is_save=True)
...
page_shop_2 = Page(name='大众点评购物店铺详情页面', fieldlist=fl_shop2)
...
page_comment_1 = Page(name='大众点评购物评论列表', fieldlist=fl_comment1, listcssselector=ListCssSelector(list_css_selector='#review-list > div.review-list-container > div.review-list-main > div.reviews-wrapper > div.reviews-items > ul > li'), mongodb=Mongodb(db=TravelDriver.db, collection=TravelDriver.comments_collection), is_save=True)

class DianpingShoppingSpider(TravelDriver):
    ...
    def get_shop_info_list(self):
        self.fast_click_first_item_page_by_partial_link_text(link_text='购物')
        time.sleep(2)
        while (True):
            self.is_ready_by_proxy_ip()
            self.switch_window_by_index(index=-1)
